...
#        ./crntb-adm.sh uninstall
        echo "Stopping legend-club-item service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting legend-club-item service..."
        nohup python index.py -log_file_prefix=./log/club.item.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup legend-club-item service success!'
